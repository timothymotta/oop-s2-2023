...
//  Given a matrix of integers, count the number of times each number 0-9 appears. Print out your results on one line in the following form:
...
void count_digits(int array[4][4]){
 int count[10] = {0};  // Initialize an array to count the occurrences of each digit

    // Iterate through the matrix and count the occurrences of each digit
    for (int i = 0; i < 4; i++) {
        for (int j = 0; j < 4; j++) {
            count[array[i][j]]++;
        }
